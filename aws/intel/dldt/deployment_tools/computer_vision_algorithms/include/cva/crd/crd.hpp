...
#ifndef CVA_CRD_CRD_HPP
#define CVA_CRD_CRD_HPP
...
#ifdef CVA_CRD_BUILD
#define CVA_CRD_EXPORT CVA_AC_SHARED_LIBRARY_EXPORT
#else
#define CVA_CRD_EXPORT CVA_AC_SHARED_LIBRARY_IMPORT
#endif
...
CVA_CRD_EXPORT ac::Version version();
...
public:
    ...
    * that uses the CPU for computations. CPU always uses FP32 model.
    */
   CVA_CRD_EXPORT static std::unique_ptr<CrossroadDetector> createCpu();

   /*! \brief Returns a pointer to a new object implementing ObjectDetectorCommon
    * that uses the GPU for computations.
    * \p precision model precision. FP32 or FP16 are supported.
    */
   CVA_CRD_EXPORT static std::unique_ptr<CrossroadDetector> createGpu(const cva::ac::Precision precision = cva::ac::Precision::FP32);

   /*! \brief Virtual destructor
    */
   virtual ~CrossroadDetector() = default;
