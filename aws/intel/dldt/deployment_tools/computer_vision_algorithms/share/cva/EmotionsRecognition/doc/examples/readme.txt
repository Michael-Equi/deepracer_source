Emotion Recognition Algorithm Samples
=====================================

General Description
-------------------

This directory contains samples for the Emotion Recognition algorithm. The following
samples are available:

* recognize - takes an image as an input and prints what emotion is most likely expressed by a person on the image.
* recognize_many - takes a list of images, recognizes an emotion on each of these images,
and outputs the most probable emotion for each image.

For more information about each sample, see the readme.txt files in the
"recognize" and "recognize_many" directories.
...
* <SAMPLE_NAME> - the name of a sample (the same as the name of its directory)
...
3. Configure the build with CMake:
   ...
   cmake -D CMAKE_PREFIX_PATH="<CVA_DIR>" \
      "<CVA_DIR>/share/cva/EmotionsRecognition/doc/examples/<EXAMPLE_NAME>"

4. Build the sample:

   cmake --build .

The build system will place the built application as cva_er_example_<SAMPLE_NAME> in the specified build directory.
...
3. Configure the build with CMake:
   ...
   cmake -G "Visual Studio 14 2015 Win64" -D CMAKE_PREFIX_PATH="<CVA_DIR>" ^
	   "<CVA_DIR>\share\cva\EmotionsRecognition\doc\examples\<SAMPLE_NAME>"
...
Release\cva_er_example_<EXAMPLE_NAME>.exe in the specified build directory.
