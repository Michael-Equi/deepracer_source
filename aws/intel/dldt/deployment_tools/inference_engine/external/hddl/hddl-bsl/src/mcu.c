...
static BSL_STATUS m_mcu_config(struct json_object* config) {
  ...
  if(strlen(m_i2c_port_or_name) == 0){
    bsl_strncpy(m_i2c_port_or_name, sizeof(m_i2c_port_or_name), MCU_DEFAULT_I2C_PORT_NAME,
...
static BSL_STATUS m_mcu_scan(int* p_device_count) {
  ...
  *p_device_count = 0;
  ...
  BSL_STATUS status = i2c_fetch_address_by_scan(m_i2c_port_or_name, m_mcu_start_addr, m_mcu_end_addr, address_list, p_device_count);
  if (status < 0) {
    return status;
  }
